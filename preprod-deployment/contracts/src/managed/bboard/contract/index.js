...
const __compactRuntime = {
  ...
  // PATCH: compact-runtime's queryLedgerState does `circuitContext.currentQueryContext = res.context`
  // after every WASM query. In onchain-runtime-v3, QueryContext.query() returns a QueryResults object
  // whose `.context` property may be undefined (the WASM binding changed shape between versions).
  // When that happens the very next state access crashes with
  //   "Cannot read properties of undefined (reading 'currentQueryContext')".
  // Fix: preserve the pre-call context whenever the base implementation clears it.
  queryLedgerState: function(circuitContext, partialProofData, program) {
    const savedQueryContext = circuitContext.currentQueryContext;
    const result = __compactRuntimeBase.queryLedgerState(circuitContext, partialProofData, program);
    if (!circuitContext.currentQueryContext) {
      circuitContext.currentQueryContext = savedQueryContext;
    }
    return result;
  },
};
  ...
  constructor(...args_0) {
    ...
    this.circuits = {
      openElection: async (...args_1) => {
        ...
        return { result: result_0, context: context, proofData: partialProofData, gasCost: context.currentGasCost };
      },
...
export const _descriptor_0_export = _descriptor_0;
